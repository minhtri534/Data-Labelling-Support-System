...
export const annotatorService = {
  ...
  async getAnnotations(annotationTaskId: string) {
    const res = await api.get(`/annotator/tasks/${annotationTaskId}/annotations`)
    return res.data
  },

  async getImageSecure(dataItemId: string): Promise<Blob> {
    const url = `/annotator/tasks/${dataItemId}/data-item/content`; 
